...
/**
 * This class is a sort of one-way Radix Set. It is similar to a Radix Tree or Patricia Trie, 
 * but instead of storing data in the nodes, it is used only as a set with no data being stored
 * in the node. It is a add-only dataset, so it does not contain a remove() method. If the
 * strings in question have a common prefix, then this will efficiently store the data. 
 * 
 * It was written as an alternative to a Bloom Filter in determining if a particular string
 * has already been seen (for DNA sequencing reads, only write out unique names, no duplicates).
 * Instead of using a probabilistic measure for false reporting, this will determine with 100% 
 * accuracy if a particular String has been seen or not.
 * 
 * In practice, this is slightly slower than a HashSet, but is more memory efficient.
 * 
 * @author mbreese
 *
 */
public class RadixSet implements Set<String>{
